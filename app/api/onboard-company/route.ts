...
function detectAtsFromUrl(url: string): AtsInfo | null {
...
function extractCountry(location: string): string {
  ...
  // "Remote" alone → no country
  if (/^remote$/i.test(loc.trim())) return '';
...
function extractSalary(rawData: any): string {
  ...
  if (rawData.pay) {
    ...
    if (min_amount || max_amount) {
      ...
      if (min_amount && max_amount) return `${currency} ${min_amount.toLocaleString()} - ${max_amount.toLocaleString()}`;
...
function parseGreenhouseJobs(data: any): ParsedJob[] {
  ...
  return jobs.map((job: any) => {
    ...
    const remoteFirst = !!(
      (job.offices && job.offices.some((o: any) => /remote/i.test(o.name || '') || /remote/i.test(o.location || ''))) ||
  ...
  return jobs.map((job: any) => {
    ...
    if (typeof job.location === 'string') {
      ...
      if (typeof job.jobLocation === 'string') {
        ...
        location = job.jobLocation.city + (job.jobLocation.country ? ', ' + job.jobLocation.country : '');
      }
    } else if (job.location?.name) {
      location = job.location.name;
    }
...
async function fetchJobsFromAts(atsInfo: AtsInfo): Promise<ParsedJob[]> {
  ...
  const text = await response.text();
  if (!response.ok) {
    throw new Error(`${atsInfo.platform} API error ${response.status}: ${text.substring(0, 200)}`);
  }
  const data = JSON.parse(text);

  switch (atsInfo.platform) {
    case 'Greenhouse': return parseGreenhouseJobs(data);
    case 'Lever': return parseLeverJobs(data);
    case 'Ashby': return parseAshbyJobs(data);
  }
}
...
async function fetchCompanyRecord(companyId: string) {
  return airtableFetch(
    `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${COMPANIES_TABLE}/${companyId}`
  );
}
...
async function getExistingJobUrls(companyName: string): Promise<Set<string>> {
  ...
  params.append('filterByFormula', `FIND("${escaped}", ARRAYJOIN({Companies}, "||") & "")`);
  params.append('fields[]', 'Job URL');

  const data = await airtableFetch(
    `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${encodeURIComponent(JOBS_TABLE)}?${params.toString()}`
  );
...
async function fetchFunctionMap(): Promise<Map<string, string>> {
  const data = await airtableFetch(
    `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${FUNCTIONS_TABLE}?fields[]=Function`
  );
...
async function createJobsBatch(
  ...
  for (let i = 0; i < jobs.length; i += 10) {
    ...
    try {
      await airtableFetch(
        `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${encodeURIComponent(JOBS_TABLE)}`,
        {
          method: 'POST',
...
function mapStage(raw: string): string | null {
  ...
  if (t.includes('ipo') || t.includes('public') || t.includes('nyse') || t.includes('nasdaq')) return 'Public';
  ...
  if (t.includes('series d') || t.includes('series e') || t.includes('series f') || t.includes('series g')) return 'Late Stage';
  return null;
}
...
async function enrichCompanyMetadata(companyName: string): Promise<{ stage?: string; size?: string }> {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    progress.companyName = companyName;
    progress.steps.push(`Fetched company: ${companyName}`);

    if (!companyName) {
      return NextResponse.json({ ...progress, error: 'Company record has no name' }, { status: 400 });
    }
    ...
    if (company.fields['Jobs API URL']) {
      ...
      if (atsInfo) progress.steps.push(`Detected ${atsInfo.platform} from existing Jobs API URL`);
    }

    // Try Website field
    if (!atsInfo && company.fields.Website) {
      atsInfo = detectAtsFromUrl(company.fields.Website);
      if (atsInfo) progress.steps.push(`Detected ${atsInfo.platform} from Website field`);
    }

    // Fall back to Perplexity: find the careers page, then detect ATS from that
    if (!atsInfo) {
      progress.steps.push('No ATS detected from existing URLs, asking Perplexity...');
      const careersUrl = await findCareersUrl(companyName);
      if (careersUrl) {
        progress.steps.push(`Perplexity found: ${careersUrl}`);
        atsInfo = detectAtsFromUrl(careersUrl);
        if (atsInfo) {
          progress.steps.push(`Detected ${atsInfo.platform} from careers URL`);
        } else {
          progress.steps.push(`URL doesn't match known ATS platforms`);
        }
      } else {
        progress.steps.push('Perplexity could not find a careers URL');
      }
    }
    ...
    progress.atsPlatform = atsInfo.platform;
    progress.atsUrl = atsInfo.apiUrl;
    ...
    progress.steps.push(`Updated company: ATS Platform = ${atsInfo.platform}, Jobs API URL saved`);

    // Step 3: Fetch jobs from ATS
    const rawJobs = await fetchJobsFromAts(atsInfo);
    progress.jobsFetched = rawJobs.length;
    progress.steps.push(`Fetched ${rawJobs.length} jobs from ${atsInfo.platform}`);
      ...
      if (Date.now() - startTime < MAX_RUNTIME_MS) {
        if (!company.fields.Stage || !company.fields.Size) {
          ...
          if (Object.keys(enrichFields).length > 0) {
            ...
            progress.companyEnriched = true;
            progress.steps.push(`Enriched company: ${JSON.stringify(enrichFields)}`);
          }
        }
      }
      return NextResponse.json({
    ...
    progress.jobsNew = newJobs.length;
    progress.jobsDuplicate = rawJobs.length - newJobs.length;
    progress.steps.push(`${newJobs.length} new jobs, ${progress.jobsDuplicate} already exist`);

    // Step 5: Create new jobs with full metadata
    if (newJobs.length > 0 && Date.now() - startTime < MAX_RUNTIME_MS) {
      const functionMap = await fetchFunctionMap();
      const created = await createJobsBatch(newJobs, companyId, functionMap);
      progress.jobsCreated = created;
      progress.steps.push(`Created ${created} job records in Airtable`);
    }
    ...
    if (Date.now() - startTime < MAX_RUNTIME_MS) {
      if (!company.fields.Stage || !company.fields.Size) {
        ...
        if (Object.keys(enrichFields).length > 0) {
          ...
          progress.steps.push('Perplexity enrichment returned no mappable data');
        }
      } else {
        progress.steps.push('Company already has Stage and Size — skipped enrichment');
      }
    }
